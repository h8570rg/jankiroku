...
test.describe("新規登録", () => {
  ...
  test("正しい情報で新規登録すると、登録ページへリダイレクトされる", async ({ page }) => {
  ...
  test("すでに使用されているメールアドレスでは登録できない", async ({ page }) => {
    await page.goto("/sign-up");
    ...
    await expect(page.getByText("このメールアドレスは使用できません")).toBeVisible();
  ...
  test("メールアドレス形式が不正ならバリデーションエラー", async ({ page }) => {
    ...
    await page.getByRole("textbox", { name: "メールアドレス" }).fill("not-an-email");
    await page.getByRole("textbox", { name: "パスワード" }).fill("password123");
    await page.getByRole("button", { name: "新規登録", exact: true }).click();
